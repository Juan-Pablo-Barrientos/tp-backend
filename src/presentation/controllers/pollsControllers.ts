...
const addPolls = async (req: any, res: any) => {
  try {
    ...
    if (!categoryId) {
      return res
        .status(400)
        .json({ msg: "categoryId field is required.", error: true });
    }
    if (!description) {
      return res
        .status(400)
        .json({ msg: "description field is required.", error: true });
    }
    if (!pollDate) {
      return res
        .status(400)
        .json({ msg: "pollDate field is required.", error: true });
    }
    ...
    if (sameDatePoll == null || sameDatePoll.getDataValue("id") == PollsID) {
      ...
      if (PollValues) {
        PollValues.forEach(async (pollValueInstance: any) => {
          if (
            pollValueInstance.description == "" &&
            pollValueInstance.id != null
          ) {
            const pollvalue = await models.PollValues.destroy({
              where: { id: pollValueInstance.id },
            });
          } else {
      ...
      return res
        .status(409)
        .json({ msg: "Poll date already taken", error: true });
  } catch (error) {
    return res.status(500).json({ msg: error, error: true });
  }
};
