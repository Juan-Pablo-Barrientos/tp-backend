...
const changePassword = async (req: any, res: any) => {
  try {
    ...
    if (!newPassword) {
      return res
        .status(400)
        .json({ msg: "new password field is required.", error: true });
    }
    if (!oldPassword) {
      return res
        .status(400)
        .json({ msg: "old password field is required.", error: true });
    }
    if (!userId) {
      return res
        .status(400)
        .json({ msg: "userId field is required.", error: true });
    }
...
const userExist = async (req: any, res: any) => {
  try {
    ...
    if (response != null) {
      return res
        .status(200)
        .json({ msg: `User exist.`, error: false, exist: true });
    } else {
      return res
        .status(200)
        .json({ msg: `User not found.`, error: false, exist: false });
    }
  } catch (error) {
    return res.status(500).json({ msg: error, error: true });
  }
...
const emailExist = async (req: any, res: any) => {
  try {
    ...
    if (response != null) {
      return res
        .status(200)
        .json({ msg: `Email exist.`, error: false, exist: true });
    } else {
      return res
        .status(200)
        .json({ msg: `Email not found.`, error: false, exist: false });
    }
  } catch (error) {
    return res.status(500).json({ msg: error, error: true });
  }
...
const addUser = async (req: any, res: any) => {
  try {
    const { name, surname, username, password, role, email, phoneNumber } =
      req.body;
    ...
    if (!surname) {
      return res
        .status(400)
        .json({ msg: "surname field is required.", error: true });
    }
    if (!username) {
      return res
        .status(400)
        .json({ msg: "username field is required.", error: true });
    }
    if (!password) {
      return res
        .status(400)
        .json({ msg: "password field is required.", error: true });
    }
    if (!email) {
      return res
        .status(400)
        .json({ msg: "email field is required.", error: true });
    }
    if (!role) {
      return res
        .status(400)
        .json({ msg: "role field is required.", error: true });
    }
    if (!phoneNumber) {
      return res
        .status(400)
        .json({ msg: "phoneNumber field is required.", error: true });
    }
    const alreadyExistingUser = await models.User.findOne({
      where: { username },
    });
...
const updateUser = async (req: any, res: any) => {
  try {
    ...
    if (img) {
      ...
      req.body.path_img =
        "https://res.cloudinary.com/clawgames/image/upload/w_1000,ar_16:9,c_fill/" +
        result.public_id;
    }
    if (user) {
      user.set(req.body);
...
const deleteUser = async (req: any, res: any) => {
  try {
    ...
    if (user) {
      ...
      res
        .status(200)
        .json({ data: user, error: false, msg: "User deleted successfully." });
    } else {
      res.status(404).json({ msg: "User not found", error: true });
    }
  } catch (error) {
