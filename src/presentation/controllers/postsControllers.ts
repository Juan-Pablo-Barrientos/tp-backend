...
const addPosts = async (req: any, res: any, next: any) => {
  ...
  try {
    ...
    if (postCreated) {
      const subscribedUsers = await models.User.findAll({
...
const updatePosts = async (req: any, res: any) => {
  try {
    ...
    if (Posts) {
      ...
      res
        .status(404)
        .json({ msg: "Posts not found", status: 404, error: true });
    }
  } catch (error) {
...
const getMostClickedPosts = async (req: any, res: any) => {
  try {
    ...
    const minusWeek = new Date(
      new Date().setDate(new Date().getDate() - 7 * numWeeks),
    );
