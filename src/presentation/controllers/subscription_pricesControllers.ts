...
const getPriceByDate = async (req: any, res: any) => {
  try {
    const response = await models.SubscriptionPrices.findOne({
      where: { effectiveDate: req.params },
    });
    if (response != null) {
...
const addPrice = async (req: any, res: any) => {
  try {
    ...
    if (!SubscriptionPrices) {
      return res
        .status(400)
        .json({ msg: "SubscriptionPrices field is required.", error: true });
    }
    if (!price) {
      return res
        .status(400)
        .json({ msg: "price field is required.", error: true });
    }
...
export { addPrice, deletePrice, getAllPrice, getPriceByDate };
